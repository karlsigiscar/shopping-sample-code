...
class ProductService: NSObject, ObservableObject {
    ...
    public func loadUserAndPromotions() {
        ...
        let userPublisher = URLSession.shared.dataTaskPublisher(for: url)
            ...
            .flatMap { user in self.loadProductsOnPromotionForUser(userID: user.id) }
            .sink(receiveCompletion: { print("Completed: \($0)") }, receiveValue: { [weak self] configuration in
    ...
    public func loadProductsOnPromotionForUser(userID: Int) -> AnyPublisher<ConfigurationResponse, Error> {
        ...
        guard let url = URL(string: "\(baseURLString)/data.json?userID = \(userID)") else{
